...
static void cmd_list_buffers(ViewerState *st) {
    if (!check_command_exists("fzf")) {
        // Fallback to old behavior if fzf not available
        def_prog_mode();
        endwin();
        printf("\n=== Buffer List ===\n\n");
    ...
    // Create temp file for buffer list
    ...
    // Write buffer list to temp file
    for (int i = 0; i < st->buffer_count; i++) {
        Buffer *b = &st->buffers[i];
        const char *indicator = (i == st->current_buffer) ? "*" : " ";
        const char *modified = b->dirty ? "[+]" : "   ";
    ...
    fflush(list_file);  // Make sure data is written
    fclose(list_file);
    ...
    // Run fzf - NOTE: no pipe, just direct file input
    char cmd[8192];
    ...
    // Parse buffer number (first field before |)
...
static int cursor_screen_row(ViewerState *st) {
    Buffer *b = &st->buffers[st->current_buffer];
    if (!st->wrap_enabled) {
        return st->cursor_line - b->scroll_offset;
    }
    int row = 0;
...
static void draw_status_bar(ViewerState *st) {
    ...
    move(max_y - 1, 0);  // Changed from max_y - 2
    clrtoeol();
    ...
    snprintf(left, sizeof(left),
             ...
             b->dirty ? " | +modified" : "");
    mvprintw(max_y - 1, 1, "%.*s", max_x - 2, left);  // Changed from max_y - 2
    ...
    if (st->mode == MODE_COMMAND) {
        ...
        if (avail > 0) {
            ...
            mvprintw(max_y - 1, start_x, "%.*s", viewlen, view);  // Changed from max_y - 2
        }
    } else if (st->status_ticks > 0 && st->status_msg[0]) {
        mvprintw(max_y - 1, max_x - (int)strlen(st->status_msg) - 2, "%s", st->status_msg);  // Changed from max_y - 2
        ...
        mvprintw(max_y - 1, max_x - (int)strlen(right) - 2, "%s", right);  // Changed from max_y - 2
    }
...
static char *fzf_pick_file(void) {
    if (!check_command_exists("fzf")) {
        return NULL;
    }
    ...
    if (has_fd) {
        ...
        snprintf(cmd, sizeof(cmd),
            "find . -type d \\( -name .git -o -name node_modules -o -name build -o -name dist -o -name .cache \\) -prune -o -type f -print 2>/dev/null "
            "| fzf --height=80%% --layout=reverse --border "
            "< /dev/tty > \"%s\" 2> /dev/tty",
            tmp_template
        );
...
static void exec_command(ViewerState *st, const char *cmd, int *running) {
    ...
    if (strcmp(tok, "qa") == 0 || strcmp(tok, "qall") == 0) {
        ...
        snprintf(msg, sizeof(msg),
                 "%d modified buffer%s (use :qa! to quit anyway)", n, (n==1?"":"s"));
        set_status(st, msg);
        return;
    }
 ...
 if (strcmp(tok, "b") == 0) {
        if (strncmp(p, "new", 3) == 0) { add_blank_buffer(st); return; }
        if (strncmp(p, "n", 1) == 0 && (p[1] == '\0' || isspace((unsigned char)p[1])))   { next_buffer(st); return; }
        if (strncmp(p, "p", 1) == 0 && (p[1] == '\0' || isspace((unsigned char)p[1])))   { prev_buffer(st); return; }
        if (strncmp(p, "add", 3) == 0 || (strncmp(p, "a", 1) == 0 && (p[1] == '\0' || isspace((unsigned char)p[1])))) {
            char *picked = fzf_pick_file();
            if (!picked) {
                set_status(st, "No file selected (fzf required)");
                return;
            }
        ...
        if (strncmp(p, "a", 1) == 0) { // shorthand :b a
            char *picked = fzf_pick_file();
            if (!picked) { set_status(st, "No file selected"); return; }
            add_buffer_from_file(st, picked);
            free(picked);
            return;
        }
...
static void cmd_show_help(ViewerState *st) {
    if (!check_command_exists("fzf")) {
        set_status(st, "fzf required for help menu");
        return;
    }

    // Create temp file for help content
    ...
    // Run fzf - same pattern as cmd_list_buffers
    char cmd[8192];
    snprintf(cmd, sizeof(cmd),
...
static void handle_command_key(ViewerState *st, int ch, int *running) {
    if (ch == 27) {
     if(curs_set(0)) curs_set(1);
     exit_command_mode(st); return; }
...
static void handle_input(ViewerState *st, int *running) {
    ...
    Buffer *b = &st->buffers[st->current_buffer];
    if (st->mode == MODE_VISUAL) {
        if (ch == 27) { st->mode = MODE_NORMAL; st->op_pending = OP_NONE; return; }
        if (ch == 'y') { visual_copy_to_clipboard(st); st->mode = MODE_NORMAL; st->op_pending = OP_NONE; return; }
    }
    if (st->g_pending) {
        st->g_pending = 0;
    ...
    if (st->op_pending != OP_NONE) {
        ...
        if (ch == 'd' && st->op_pending == OP_DELETE) {
            ...
            for (int i = st->cursor_line; i < b->line_count - 1; i++) b->lines[i] = b->lines[i + 1];
            b->lines[b->line_count - 1] = NULL;
            b->line_count--;
            if (b->line_count <= 0) { b->line_count = 1; b->lines[0] = strdup(""); st->cursor_line = 0; }
            if (st->cursor_line >= b->line_count) st->cursor_line = b->line_count - 1;
            st->cursor_col = 0;
            b->dirty = 1;
    ...
    switch (ch) {
        ...
        case 'G':
            ...
            st->cursor_col = 0;
            ensure_cursor_visible(st);
            if (st->mode == MODE_VISUAL) st->vis_end = st->cursor_line;
            return;
        case 'h':
        case KEY_LEFT:
            st->free_scroll = 0;
